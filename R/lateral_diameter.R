...
lateral_diameter <- function(q_var_lateral = 10, h_fim_lateral, s_lateral, s_ini_lateral = s_lateral, n_lateral, dec_lateral, coef_em, exp_em, rc, ...) {
  f <- function(d_lateral) {
    r <- do.call(lateral_profile, list(h_fim_lateral, d_lateral, s_lateral, s_ini_lateral, n_lateral, dec_lateral, coef_em, exp_em, rc))
    return((max(r$q_em) - min(r$q_em)) / max(r$q_em) * 100 - q_var_lateral)
  }

  d_lateral <- bisection(f, ...)
  ...
  res_x <- do.call(lateral_profile, list(h_fim_lateral, d_lateral$root, s_lateral, s_ini_lateral, n_lateral, dec_lateral, coef_em, exp_em, rc))
